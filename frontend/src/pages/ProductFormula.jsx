...
const ProductFormula = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Debug chosen formulas structure
      console.log('Chosen formulas sample:', chosenRes?.[0]);
      console.log('Sample KP value:', chosenRes?.[0]?.KP);
      console.log('Is sample KP empty string?', chosenRes?.[0]?.KP === '');
      console.log('Is sample KP null?', chosenRes?.[0]?.KP === null);
  ...
  const loadProductFormulas = async (productId) => {
    try {
      setLoading(true);
      console.log('Loading formulas for product ID:', productId);
      
      const formulas = await api.products.getFormulaById(productId);
      console.log('Raw formula data for product', productId, ':', formulas);
      
      // Check if we got any formulas
      if (!formulas || formulas.length === 0) {
        console.log('No formulas found for product', productId);
        setProductFormulas({ PI: [], PS: [], KP: [], KS: [] });
        return;
      }
      
      console.log('Sample formula object:', formulas[0]);
      console.log('Sample formula PPI_SubID:', formulas[0]?.PPI_SubID);
      console.log('Is PPI_SubID empty string?', formulas[0]?.PPI_SubID === '');
      console.log('Is PPI_SubID null?', formulas[0]?.PPI_SubID === null);
      ...
      console.log('Grouped formulas:', grouped);
      
      // Check if any group has formulas
      const hasAnyFormulas = Object.values(grouped).some(group => group.length > 0);
      console.log('Has any formulas:', hasAnyFormulas);
  ...
  const handleEdit = async (formula) => {
    ...
    // Debug the form data setup
    console.log('Edit form data setup for product:', formula.Product_ID);
    console.log('Formula values:', {
      PI: formula.PI, PS: formula.PS, KP: formula.KP, KS: formula.KS
    });
  ...
  const handleFormulaSelect = (type, formulaId) => {
    console.log(`[DEBUG] Formula selection: ${type} = "${formulaId}" (was: "${formData[type.toLowerCase()]}")`);
