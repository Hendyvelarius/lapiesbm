...
async function deploy() {
    try {
        ...
        // Deploy v9 procedure
        console.log('Reading v9 script...');
        const script = fs.readFileSync('./migrations/010_sp_COGS_Calculate_HPP_Actual_v9.sql', 'utf8');
        
        // Split by GO and execute each batch
        const batches = script.split(/\nGO\s*\n/i).filter(b => b.trim());
        
        console.log(`Deploying sp_COGS_Calculate_HPP_Actual v9 (${batches.length} batches)...`);
        ...
        console.log('v9 deployed successfully');
        await sql.close();
    } catch (err) {
