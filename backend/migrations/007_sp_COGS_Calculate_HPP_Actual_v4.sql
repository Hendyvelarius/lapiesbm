-- =====================================================================
-- Stored Procedure: sp_COGS_Calculate_HPP_Actual (v4)
...
--   - Without this fix, batch 77016 in 2026 was picking up MRs from 2016!
-- Changes in v3:
...
BEGIN
    ...
    BEGIN
        BEGIN TRY
            ...
            -- v4 FIX: Use subquery with TOP 1 to avoid multiplication from multiple periode rows
            LEFT JOIN (
                SELECT ITEM_ID, ITEM_TYPE
                FROM M_COGS_STD_HRG_BAHAN
                WHERE Periode = @GroupPeriode
            ) mst ON d.MR_ItemID = mst.ITEM_ID
            ...
            -- v4 FIX: Use subquery with TOP 1 to avoid multiplication from multiple periode rows
            LEFT JOIN (
                SELECT Product_ID, PPI_ItemID, PPI_QTY
