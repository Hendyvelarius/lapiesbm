-- ============================================================
-- Script 20: Copy t_ttba_manufacturing_detail from LIVE to DEV
-- Source: LIVE_SERVER (192.168.1.21)
-- Target: DEV (this server, 192.168.1.49)
-- Expected rows: ~337,000+
-- ============================================================
...
WHERE fk.referenced_object_id = OBJECT_ID('t_ttba_manufacturing_detail');
...
SELECT @sql_out = @sql_out + 
    'ALTER TABLE [' + OBJECT_SCHEMA_NAME(fk.parent_object_id) + '].[' + OBJECT_NAME(fk.parent_object_id) + '] NOCHECK CONSTRAINT [' + fk.name + '];' + CHAR(13)
FROM sys.foreign_keys fk
WHERE fk.parent_object_id = OBJECT_ID('t_ttba_manufacturing_detail');
...
BEGIN TRANSACTION;
BEGIN TRY
    DELETE FROM [dbo].[t_ttba_manufacturing_detail];
    PRINT 'DELETE completed. Inserting from LIVE...';

    INSERT INTO [dbo].[t_ttba_manufacturing_detail]
    SELECT * FROM [LIVE_SERVER].[lapifactory].[dbo].[t_ttba_manufacturing_detail];
...
-- PART 4: Verify
SELECT 'DEV' AS [Server], COUNT(*) AS [RowCount] FROM [dbo].[t_ttba_manufacturing_detail]
UNION ALL
SELECT 'LIVE', COUNT(*) FROM [LIVE_SERVER].[lapifactory].[dbo].[t_ttba_manufacturing_detail];
