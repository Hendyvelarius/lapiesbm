-- ============================================================
-- Script 21: Copy tmp_spLapProduksi_GWN_ReleaseQA from LIVE to DEV
...
WHERE fk.referenced_object_id = OBJECT_ID('tmp_spLapProduksi_GWN_ReleaseQA');
...
SELECT @sql_out = @sql_out + 
    'ALTER TABLE [' + OBJECT_SCHEMA_NAME(fk.parent_object_id) + '].[' + OBJECT_NAME(fk.parent_object_id) + '] NOCHECK CONSTRAINT [' + fk.name + '];' + CHAR(13)
FROM sys.foreign_keys fk
WHERE fk.parent_object_id = OBJECT_ID('tmp_spLapProduksi_GWN_ReleaseQA');
...
BEGIN TRANSACTION;
BEGIN TRY
    DELETE FROM [dbo].[tmp_spLapProduksi_GWN_ReleaseQA];
    PRINT 'DELETE completed. Inserting from LIVE...';

    INSERT INTO [dbo].[tmp_spLapProduksi_GWN_ReleaseQA]
    SELECT * FROM [LIVE_SERVER].[lapifactory].[dbo].[tmp_spLapProduksi_GWN_ReleaseQA];
...
-- PART 4: Verify
SELECT 'DEV' AS [Server], COUNT(*) AS [RowCount] FROM [dbo].[tmp_spLapProduksi_GWN_ReleaseQA]
UNION ALL
SELECT 'LIVE', COUNT(*) FROM [LIVE_SERVER].[lapifactory].[dbo].[tmp_spLapProduksi_GWN_ReleaseQA];
