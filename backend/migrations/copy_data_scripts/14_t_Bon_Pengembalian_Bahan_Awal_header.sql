-- ============================================================
-- Script 14: Copy t_Bon_Pengembalian_Bahan_Awal_header from LIVE to DEV
-- Source: LIVE_SERVER (192.168.1.21)
-- Target: DEV (this server, 192.168.1.49)
-- Expected rows: ~61,000+
--
-- ** Run this BEFORE script 13 (Detail) **
-- ============================================================
...
WHERE fk.referenced_object_id = OBJECT_ID('t_Bon_Pengembalian_Bahan_Awal_header');
...
SELECT @sql_out = @sql_out + 
    'ALTER TABLE [' + OBJECT_SCHEMA_NAME(fk.parent_object_id) + '].[' + OBJECT_NAME(fk.parent_object_id) + '] NOCHECK CONSTRAINT [' + fk.name + '];' + CHAR(13)
FROM sys.foreign_keys fk
WHERE fk.parent_object_id = OBJECT_ID('t_Bon_Pengembalian_Bahan_Awal_header');
...
BEGIN TRANSACTION;
BEGIN TRY
    DELETE FROM [dbo].[t_Bon_Pengembalian_Bahan_Awal_header];
    PRINT 'DELETE completed. Inserting from LIVE...';

    INSERT INTO [dbo].[t_Bon_Pengembalian_Bahan_Awal_header]
    SELECT * FROM [LIVE_SERVER].[lapifactory].[dbo].[t_Bon_Pengembalian_Bahan_Awal_header];
...
-- PART 4: Verify
SELECT 'DEV' AS [Server], COUNT(*) AS [RowCount] FROM [dbo].[t_Bon_Pengembalian_Bahan_Awal_header]
UNION ALL
SELECT 'LIVE', COUNT(*) FROM [LIVE_SERVER].[lapifactory].[dbo].[t_Bon_Pengembalian_Bahan_Awal_header];
