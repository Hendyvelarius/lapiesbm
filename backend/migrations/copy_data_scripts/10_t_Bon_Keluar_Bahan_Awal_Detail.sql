-- ============================================================
-- Script 10: Copy t_Bon_Keluar_Bahan_Awal_Detail from LIVE to DEV
-- Source: LIVE_SERVER (192.168.1.21)
-- Target: DEV (this server, 192.168.1.49)
-- Expected rows: ~823,000+
-- ============================================================

-- PART 1: Disable ALL FK constraints (incoming + outgoing)
-- Incoming: other tables referencing THIS table
...
WHERE fk.referenced_object_id = OBJECT_ID('t_Bon_Keluar_Bahan_Awal_Detail');
IF @sql_in <> '' EXEC sp_executesql @sql_in;
PRINT 'Incoming FK constraints disabled.';

-- Outgoing: THIS table referencing other tables
DECLARE @sql_out NVARCHAR(MAX) = N'';
SELECT @sql_out = @sql_out + 
    'ALTER TABLE [' + OBJECT_SCHEMA_NAME(fk.parent_object_id) + '].[' + OBJECT_NAME(fk.parent_object_id) + '] NOCHECK CONSTRAINT [' + fk.name + '];' + CHAR(13)
FROM sys.foreign_keys fk
WHERE fk.parent_object_id = OBJECT_ID('t_Bon_Keluar_Bahan_Awal_Detail');
...
BEGIN TRANSACTION;
BEGIN TRY
    DELETE FROM [dbo].[t_Bon_Keluar_Bahan_Awal_Detail];
    PRINT 'DELETE completed. Inserting from LIVE...';

    INSERT INTO [dbo].[t_Bon_Keluar_Bahan_Awal_Detail]
    SELECT * FROM [LIVE_SERVER].[lapifactory].[dbo].[t_Bon_Keluar_Bahan_Awal_Detail];
...
-- PART 4: Verify
SELECT 'DEV' AS [Server], COUNT(*) AS [RowCount] FROM [dbo].[t_Bon_Keluar_Bahan_Awal_Detail]
UNION ALL
SELECT 'LIVE', COUNT(*) FROM [LIVE_SERVER].[lapifactory].[dbo].[t_Bon_Keluar_Bahan_Awal_Detail];
