-- ============================================================
-- Script 15: Copy t_DNc_Manufacturing from LIVE to DEV
-- Source: LIVE_SERVER (192.168.1.21)
-- Target: DEV (this server, 192.168.1.49)
-- Expected rows: ~89,000+
-- ============================================================
...
WHERE fk.referenced_object_id = OBJECT_ID('t_DNc_Manufacturing');
...
SELECT @sql_out = @sql_out + 
    'ALTER TABLE [' + OBJECT_SCHEMA_NAME(fk.parent_object_id) + '].[' + OBJECT_NAME(fk.parent_object_id) + '] NOCHECK CONSTRAINT [' + fk.name + '];' + CHAR(13)
FROM sys.foreign_keys fk
WHERE fk.parent_object_id = OBJECT_ID('t_DNc_Manufacturing');
...
BEGIN TRANSACTION;
BEGIN TRY
    DELETE FROM [dbo].[t_DNc_Manufacturing];
    PRINT 'DELETE completed. Inserting from LIVE...';

    INSERT INTO [dbo].[t_DNc_Manufacturing]
    SELECT * FROM [LIVE_SERVER].[lapifactory].[dbo].[t_DNc_Manufacturing];
...
-- PART 4: Verify
SELECT 'DEV' AS [Server], COUNT(*) AS [RowCount] FROM [dbo].[t_DNc_Manufacturing]
UNION ALL
SELECT 'LIVE', COUNT(*) FROM [LIVE_SERVER].[lapifactory].[dbo].[t_DNc_Manufacturing];
