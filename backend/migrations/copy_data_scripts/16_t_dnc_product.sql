...
WHERE fk.referenced_object_id = OBJECT_ID('t_dnc_product');
...
SELECT @sql_out = @sql_out + 
    'ALTER TABLE [' + OBJECT_SCHEMA_NAME(fk.parent_object_id) + '].[' + OBJECT_NAME(fk.parent_object_id) + '] NOCHECK CONSTRAINT [' + fk.name + '];' + CHAR(13)
FROM sys.foreign_keys fk
WHERE fk.parent_object_id = OBJECT_ID('t_dnc_product');
...
BEGIN TRANSACTION;
BEGIN TRY
    DELETE FROM [dbo].[t_dnc_product];
    PRINT 'DELETE completed. Inserting from LIVE...';

    INSERT INTO [dbo].[t_dnc_product]
    SELECT * FROM [LIVE_SERVER].[lapifactory].[dbo].[t_dnc_product];
...
-- PART 4: Verify
SELECT 'DEV' AS [Server], COUNT(*) AS [RowCount] FROM [dbo].[t_dnc_product]
UNION ALL
SELECT 'LIVE', COUNT(*) FROM [LIVE_SERVER].[lapifactory].[dbo].[t_dnc_product];
