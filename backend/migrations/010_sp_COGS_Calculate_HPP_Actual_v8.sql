-- =====================================================================
-- Stored Procedure: sp_COGS_Calculate_HPP_Actual (v8)
-- Purpose: Calculate true batch cost based on actual material prices
-- Changes in v8:
--   - CRITICAL FIX: Granulate output date filtering
--     Batch numbers (like 'ä0155') can be reused across years.
--     Previous versions summed ALL t_dnc_manufacturing records without
--     date filtering, causing old obsolete records to be included.
--     Example: Batch ä0155 had records from 2015 (39,240g) AND 2025 (155,640g)
--     v7 incorrectly calculated: 194,880g (summed both years)
--     v8 correctly filters by YEAR(MR_Date) to get: 155,640g (2025 only)
--   - Added @GranulateMRDate, @GranulateMRYear variables
--   - Filter DNc_ReleaseQTY sum by matching YEAR(DNc_Date) = @GranulateMRYear
-- Changes in v7:
--   - NEW: Granulate material costing - trace back to raw materials
--     When a material is a granulate (Item_ID LIKE 'ä%'), calculate its
--     actual cost by tracing the granulate production batch and summing
--     the raw materials used to produce it from actual PO prices
--   - Added granulate tracking columns to detail
--   - Added Total_Cost_Granulate, Granulate_Count to header summary
--   - Price_Source = 'GRANULATE_CALC' for calculated granulate costs
--   - Granulates are included in BB (Bahan Baku) cost category
-- Changes in v6:
...
BEGIN
    ...
    BEGIN
        BEGIN TRY
            ...
            -- =========================================
            -- v7: Process GRANULATE materials
            -- Calculate cost by tracing back to raw materials
            -- =========================================
            DECLARE @GranulateRowNum INT;
            DECLARE @GranulateMRDNcNo VARCHAR(100);  -- The MR_DNcNo from detail (e.g., '2100/25/ä7/09')
            DECLARE @GranulateDNCBatchNo VARCHAR(50); -- The actual batch no (e.g., 'ä7095')
            DECLARE @GranulateItemID VARCHAR(50);
            DECLARE @GranulateRawCost DECIMAL(18,4);
            DECLARE @GranulateOutputQty DECIMAL(18,4);
            DECLARE @GranulateCostPerGram DECIMAL(18,6);
            DECLARE @GranulateMRNo VARCHAR(100);
            -- v8: Add variable to track MR date for filtering output records
            DECLARE @GranulateMRDate DATETIME;
            DECLARE @GranulateMRYear INT;
            
            -- Pre-fetch currency rates for granulate calculation (before cursor)
            DECLARE @GranRateUSD DECIMAL(18,4), @GranRateEUR DECIMAL(18,4);
            SELECT TOP 1
                @GranRateUSD = USD, @GranRateEUR = EUR
            FROM m_COGS_Daily_Currency
            WHERE date <= @CurrentTempelLabel
            ORDER BY date DESC;
            ...
            DECLARE granulate_cursor CURSOR LOCAL FAST_FORWARD FOR
                SELECT RowNum, MR_DNcNo, Item_ID  -- MR_DNcNo is stored in Granulate_Batch
                FROM #Materials
            ...
            BEGIN
                -- Initialize
                SET @GranulateRawCost = 0;
                SET @GranulateOutputQty = 0;
                SET @GranulateCostPerGram = 0;
                SET @GranulateMRNo = NULL;
                SET @GranulateDNCBatchNo = NULL;
                -- v8: Reset date variables
                SET @GranulateMRDate = NULL;
                SET @GranulateMRYear = NULL;
                
                IF @Debug = 1
                BEGIN
                    SET @Msg = '  Processing granulate: ' + @GranulateItemID + ' MR_DNcNo ' + ISNULL(@GranulateMRDNcNo, 'NULL');
                    PRINT @Msg;
                END
                
                -- Step 1: Get the actual batch number from t_dnc_manufacturing
                -- MR_DNcNo (e.g., '2100/25/ä7/09') matches DNc_No in t_dnc_manufacturing
                -- DNC_BatchNo (e.g., 'ä7095') is the granulate batch number
                SELECT TOP 1 @GranulateDNCBatchNo = DNC_BatchNo
                FROM t_dnc_manufacturing
                WHERE DNc_No = @GranulateMRDNcNo;
                
                IF @Debug = 1 AND @GranulateDNCBatchNo IS NOT NULL
                BEGIN
                    SET @Msg = '    Found granulate batch: ' + @GranulateDNCBatchNo;
                    PRINT @Msg;
                END
                
                -- Step 2: Find MR that produced this granulate batch
                -- The MR has MR_BatchNo = granulate batch (e.g., 'ä7095')
                -- v8: Also capture the MR_Date to filter output records by matching year
                IF @GranulateDNCBatchNo IS NOT NULL
                BEGIN
                    SELECT TOP 1 
                        @GranulateMRNo = MR_No,
                        @GranulateMRDate = MR_Date,
                        @GranulateMRYear = YEAR(MR_Date)
                    FROM t_Bon_Keluar_Bahan_Awal_Header
                    WHERE MR_BatchNo = @GranulateDNCBatchNo
                      AND MR_ProductID LIKE N'ä%';
                END
                
                IF @GranulateMRNo IS NOT NULL
                BEGIN
                    -- Calculate raw material cost from that MR
                    -- Sum: (qty_used / 1000) * PO_UnitPrice * exchange_rate
                    -- Using pre-fetched currency rates to avoid subquery in aggregate
                    SELECT @GranulateRawCost = SUM(
                    ...
                    LEFT JOIN t_ttba_manufacturing_detail t ON dm.DNc_TTBANo = t.TTBA_No AND dm.DNc_TTBASeqID = t.TTBA_SeqID
                    LEFT JOIN t_PO_Manufacturing_Detail p ON t.TTBA_SourceDocNo = p.PO_No AND t.TTBA_SourceDocSeqID = p.PO_SeqID
                    WHERE h.MR_No = @GranulateMRNo;
                    
                    -- Get output quantity from t_dnc_manufacturing using the batch number
                    -- v8 CRITICAL FIX: Filter by the same year as the MR to prevent
                    -- including old/obsolete batch records with the same batch number
                    -- (Batch numbers can be reused across years, like 'ä0155' in 2015 and 2025)
                    SELECT @GranulateOutputQty = SUM(ISNULL(DNc_ReleaseQTY, 0))
                    FROM t_dnc_manufacturing
                    WHERE DNC_BatchNo = @GranulateDNCBatchNo
                      AND YEAR(DNc_Date) = @GranulateMRYear;
                    
                    -- Calculate cost per gram
                    IF ISNULL(@GranulateOutputQty, 0) > 0
                    BEGIN
                        SET @GranulateCostPerGram = @GranulateRawCost / @GranulateOutputQty;
                    END
                    
                    IF @Debug = 1
                    BEGIN
                        SET @Msg = '    MR: ' + @GranulateMRNo + ', RawCost: ' + CAST(ISNULL(@GranulateRawCost,0) AS VARCHAR) 
                                 + ', Output: ' + CAST(ISNULL(@GranulateOutputQty,0) AS VARCHAR)
                                 + ', Cost/g: ' + CAST(ISNULL(@GranulateCostPerGram,0) AS VARCHAR);
                        PRINT @Msg;
                    END
                    
                    -- Update material record with actual batch number
                    UPDATE #Materials
                    SET Granulate_MR_No = @GranulateMRNo,
                        Granulate_Batch = @GranulateDNCBatchNo,  -- Store the actual batch no like 'ä7095'
                        Granulate_Raw_Material_Cost = @GranulateRawCost,
                        Granulate_Output_Qty = @GranulateOutputQty,
                        Granulate_Cost_Per_Gram = @GranulateCostPerGram,
                        -- Set the unit price as cost per gram (usage is in grams)
                        Unit_Price = @GranulateCostPerGram,
                        Currency_Original = 'IDR',
                        Price_Source = 'GRANULATE_CALC',
                        Price_Source_Level = 0,
                        -- For granulate, qty is already in grams, price is per gram
                    ...
                    SET @TotalCostGranulate = @TotalCostGranulate + (
                        SELECT Qty_Used * @GranulateCostPerGram FROM #Materials WHERE RowNum = @GranulateRowNum
                    );
                END
            ...
            -- v7: Include granulate tracking columns
            -- =========================================
            ...
            SELECT 
                ...
                CASE 
                    ...
                    ELSE Unit_Price * CASE Currency_Original
                        WHEN 'USD' THEN ISNULL(@RateUSD, 1)
                        WHEN 'EUR' THEN ISNULL(@RateEUR, 1)
                        WHEN 'CHF' THEN ISNULL(@RateCHF, 1)
                        WHEN 'SGD' THEN ISNULL(@RateSGD, 1)
    ...
    PRINT '';
    PRINT '========================================';
    PRINT 'HPP Actual Calculation Summary (v7)';
    PRINT '========================================';
...
PRINT 'Stored procedure sp_COGS_Calculate_HPP_Actual v7 created successfully';
GO
