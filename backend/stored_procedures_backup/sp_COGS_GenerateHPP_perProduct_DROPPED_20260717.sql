--select * from t_COGS_HPP_Product_Header where Product_ID='01LFXB'
CREATE procedure [dbo].[sp_COGS_GenerateHPP_perProduct] @Periode as varchar(4),@View as varchar=1, @generate as varchar(1), 
@_product_id as varchar(255)
as 
set nocount on
--exec [sp_COGS_GenerateHPP_perProduct] '2025', 0,1,'01LFXB'
--exec sp_COGS_GenerateHPP '2025','0','1'
--declare @periode varchar(25) = '2025'
--declare @view varchar(25)= 1 
--declare @generate varchar(25) = 0
if(@View=0 and @generate=1)
begin
	delete t_COGS_HPP_Product_Header from t_COGS_HPP_Product_Header a join M_COGS_PRODUCT_FORMULA_FIX b on a.Product_ID=b.product_id and a.Periode=b.Periode where a.Periode=@Periode and a.Product_ID=@_product_id and isnull(b.isLock,0)=0
	delete t_COGS_HPP_Product_Detail_Formula from t_COGS_HPP_Product_Detail_Formula a join M_COGS_PRODUCT_FORMULA_FIX b on a.Product_ID=b.product_id and a.Periode=b.Periode where a.Periode=@Periode and a.Product_ID=@_product_id and isnull(b.isLock,0)=0
end
...
null
from (select * from vw_COGS_Product_Group where periode=@periode) a left join (select * from vw_COGS_Pembebanan where group_periode=@Periode) b 
...
where a.Group_ProductID=@_product_id and ff.Periode=@Periode and isnull(ff.isLock,0)=0
 
-- Declare the cursor
DECLARE C_Product_List CURSOR FOR
	select pk_id, product_id, pi, ps, kp, ks, std_output from M_COGS_PRODUCT_FORMULA_FIX where periode=@periode
	and product_id=@_product_id and isnull(islock,0)=0
...
BEGIN
    ...
    select	@periode, [Product_ID],[Product_Name],[PPI_SubID],[TypeName],[TypeCode],[Source],
           [Default],[PPI_SeqID],[PPI_ItemID],[PPI_UnitID],[PPI_QTY],[DefaultCOGS],
			...
			(select * from M_COGS_STD_HRG_BAHAN where periode=@Periode) b on a.ppi_itemid=b.ITEM_ID where product_id=@product_id and ppi_subid=@pi and typecode='PI' order by ppi_seqid
    ...
    select	@periode, [Product_ID],[Product_Name],[PPI_SubID],[TypeName],[TypeCode],[Source],
           [Default],[PPI_SeqID],[PPI_ItemID],[PPI_UnitID],[PPI_QTY],[DefaultCOGS],
			...
			a.UnitPrice,
			b.ITEM_TYPE from vw_COGS_FORMULA_List_detail a join 
			(select * from M_COGS_STD_HRG_BAHAN where periode=@Periode) b on a.ppi_itemid=b.ITEM_ID where product_id=@product_id and ppi_subid=@ps and typecode='PS' order by ppi_seqid
    ...
    select	@periode, [Product_ID],[Product_Name],[PPI_SubID],[TypeName],[TypeCode],[Source],
           [Default],[PPI_SeqID],[PPI_ItemID],[PPI_UnitID],[PPI_QTY],[DefaultCOGS],
			...
			a.UnitPrice,
			b.ITEM_TYPE from vw_COGS_FORMULA_List_detail a join 
			(select * from M_COGS_STD_HRG_BAHAN where periode=@Periode) b on a.ppi_itemid=b.ITEM_ID where product_id=@product_id and ppi_subid=@kp and typecode='KP' order by ppi_seqid
...
into #tmp_utk_granul
from #tmp a join m_product b on a.product_id=b.product_id
where Product_Name like '%granulat%' or Product_Name like '%pelarut%' or b.Product_ID in ('H4','CQ')
