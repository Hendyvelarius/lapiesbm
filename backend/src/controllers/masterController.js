...
class MasterController {
    ...
    static async bulkImportGenerikGroups(req, res) {
        try {
            const { generikData, userId = "system" } = req.body;
            
            // Validate required fields
            if (!generikData || !Array.isArray(generikData) || generikData.length === 0) {
                return res.status(400).json({
                    success: false,
                    message: 'Missing required field: generikData must be a non-empty array'
                });
            }
            
            // Validate each row in the data
            for (let i = 0; i < generikData.length; i++) {
                const row = generikData[i];
                
                // Check required fields - productName not needed according to user
                ...
                if (row.pnCategoryName !== 'Produk Generik') {
                    return res.status(400).json({
                        success: false,
                        message: `Row ${i + 1}: Only Generik products are allowed in bulk import`
                    });
                }
                
                // Validate numeric fields
                const numericFields = ['pnCategory', 'mhtBB', 'mhtBK', 'mhAnalisa', 'kwhMesin'];
            ...
            const insertData = generikData.map(row => ({
                ...
                pnCategory: parseInt(row.pnCategory),
                pnCategoryName: row.pnCategoryName,
                manHourPros: null, // Keep as null - handled automatically
                manHourPack: null, // Keep as null - handled automatically
                rendemen: null,    // Keep as null - handled automatically
                dept: null,        // Keep as null - handled automatically
                mhtBB: parseFloat(row.mhtBB) || 0,
                mhtBK: parseFloat(row.mhtBK) || 0,
                mhAnalisa: parseFloat(row.mhAnalisa) || 0,
                kwhMesin: parseFloat(row.kwhMesin) || 0
            }));
            
            const insertResult = await bulkInsertGenerikGroups(insertData, userId);
            
            res.status(200).json({
                success: true,
                message: `Bulk import completed successfully. Deleted ${deleteResult.rowsAffected} old records, inserted ${insertResult.rowsAffected} new records.`,
                data: {
